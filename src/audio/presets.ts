...
export type Preset = {
  ...
  color: string
  emoji: string
  build: (target: Tone.InputNode) => PresetInstance
}
...
// Each build() creates fresh synths so per-pad effects don't bleed across pads.

// --- BOOM (Vine-style sub bass thump) ---
function buildBoom(target: Tone.InputNode): PresetInstance {
...
function buildAirhorn(target: Tone.InputNode): PresetInstance {
...
function buildTada(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.PolySynth(Tone.Synth, {
...
function buildOof(target: Tone.InputNode): PresetInstance {
...
function buildBruh(target: Tone.InputNode): PresetInstance {
...
function buildWow(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.Synth({
    oscillator: { type: 'sine' },
...
function buildYeah(target: Tone.InputNode): PresetInstance {
  const noise = new Tone.Noise('pink').start()
  const filt = new Tone.Filter(1200, 'bandpass', -24)
  filt.Q.value = 8
  const gain = new Tone.Gain(0).connect(target)
  noise.connect(filt)
  filt.connect(gain)
  return {
    trigger: (time, vel = 1, semis = 0) => {
...
function buildHuh(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.Synth({
    oscillator: { type: 'square' },
...
function buildLazer(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.Synth({
    oscillator: { type: 'sawtooth' },
...
// --- COIN (8-bit chime) ---
function buildCoin(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.Synth({
    oscillator: { type: 'square' },
...
function buildPop(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.MembraneSynth({
    pitchDecay: 0.01,
    octaves: 6,
...
// --- BELL (FM bell) ---
function buildBell(target: Tone.InputNode): PresetInstance {
...
function buildAlarm(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.Synth({
    oscillator: { type: 'square' },
...
function buildSiren(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.Synth({
    oscillator: { type: 'triangle' },
...
// --- DING (high sine bell) ---
function buildDing(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.FMSynth({
    harmonicity: 2,
    modulationIndex: 6,
...
// --- ERROR (square buzzer) ---
function buildError(target: Tone.InputNode): PresetInstance {
  const synth = new Tone.Synth({
    oscillator: { type: 'square' },
